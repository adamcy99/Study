...
# your task is to calculate how many ways exactly one digit could be removed from one of 
# the strings so that s is lexicographically smaller than t after the removal. 
# Note that we are removing only a single instance of a single digit, 
# rather than all instances (eg: removing 1 from the string a11b1c could result
# in a1b1c or a11bc, but not abc).
